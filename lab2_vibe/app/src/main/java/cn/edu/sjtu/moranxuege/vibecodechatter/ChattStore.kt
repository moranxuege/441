...
object ChattStore {
    ...
    private const val serverUrl = "https://8.160.114.186/"

    fun postChatt(context: Context, chatt: Chatt, completion: (Boolean) -> Unit) {
        val jsonObj = mapOf(
        ...
        val postRequest = JsonObjectRequest(Request.Method.POST,
            ...
            { error ->
                Log.e("postChatt", error.localizedMessage ?: "JsonObjectRequest error")
                completion(false)
            }
        )
